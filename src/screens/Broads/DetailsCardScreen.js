...
import { updateTaskCardAPI, addCommentAPI, getUser, updateBoardDetailsAPI, getBoardDetailsAPI } from '../../apis/API';
...
const DetailsCardScreen = () => {
    ...
    useEffect(() => {
        const fetchUserData = async () => {
            try {
                ...
                setIsLoading(false); // Đánh dấu là đã tải xong dữ liệu, dù thành công hay thất bại
            }
        };
    ...
    useEffect(() => {
        const fetchCardData = async () => {
            const updatedBoard = await getBoardDetailsAPI(boardId);
            const updatedCardDetails = updatedBoard?.columns?.flatMap(column => column.cards).find(c => c._id === card._id);
            setCardData(updatedCardDetails);
        };

        fetchCardData();
    }, [card._id, /* other dependencies */]);
    const handleSendComment = async () => {
        ...
        try {
            ...
            // Fetch updated card details after adding a comment
            const updatedBoard = await getBoardDetailsAPI(boardId);
            const updatedCardDetails = updatedBoard?.columns?.flatMap(column => column.cards).find(c => c._id === card._id);
            console.log(updatedCardDetails)
            console.log(updatedBoard)
            // Update cardData to trigger re-render
            setCardData(updatedCardDetails);
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                <View style={{ flexDirection: 'row', justifyContent: 'center', alignItems: 'center' }}>
                    ...
                    <UIIconButton
                        ...
                        iconSize={24}
                        onPress={() => {
                            navigation.replace('DetailsCardScreen', { card, boardId });
                        }}
                    />
